...
global_model.classifier = nn.Linear(num_ftrs, 3)

state_dict = torch.load('./TrainedModels/densenet_emotiw_pretrainemotic_lr001.pt', map_location=lambda storage, loc: storage)
...
global_model.classifier = nn.Linear(num_ftrs, 256)
nn.init.kaiming_normal_(global_model.classifier.weight)
global_model.classifier.bias.data.fill_(0.01)

non_align_model = torch.load(non_aligned_path, map_location=lambda storage, loc: storage).module
# non_align_model = nn.DataParallel(non_align_model)
# print(non_align_model.fc6)
# num_ftrs = non_align_model.fc6.in_features
# non_align_model.fc = nn.Linear(num_ftrs, 10)
# non_align_model.load_state_dict(torch.load('./TrainedModels/non_aligned_pretrain.pt'))
# num_ftrs = non_align_model.fc.in_features
non_align_model.fc6 = nn.Linear(512, 256)
# print(list(non_align_model.children()))
nn.init.kaiming_normal_(non_align_model.fc6.weight)
non_align_model.fc6.bias.data.fill_(0.01)
...
model_ft = FaceAttention(global_model, non_align_model)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model, './TrainedModels/FaceAttention_AlignedModel_FullTrain_4para_lr001_dropout_BN_SoftmaxLr01_EmotiC')
