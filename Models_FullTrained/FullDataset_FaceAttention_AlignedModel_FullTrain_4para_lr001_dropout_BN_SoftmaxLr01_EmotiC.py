...
class EmotiWDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #SAMPLE
        sample = {'image': image, 'features': features2, 'label':labeldict[labelname], 'numberFaces': numberFaces}
        return sample
...
state_dict = torch.load('../TrainedModels/TrainDataset/densenet_emotiw_pretrainemotic_lr001.pt', map_location=lambda storage, loc: storage)
...
class FaceAttention(nn.Module):
    ...
    def forward(self, image, face_features_initial, numberFaces, labels):
        ...
        for j in range(batch_size):
            ...
            face_features[j, :, :] = self.align_model_dropout(self.align_model.forward(face, labels))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model, '../TrainedModels/FullDataset/FaceAttention_AlignedModel_FullTrain_4para_lr001_dropout_BN_SoftmaxLr01_EmotiC')
